    ...
    if(word_index>=WINDOW_SIZE & word_index<(corpus_size-10)):
        ...
        while counter<WINDOW_SIZE:
            ...
            if word in vocabWordsDict:
                ...
                if((word_index+counter+2)%100000==0):
                    sys.stdout.write("\n -------------------------------- \n")
...
def initialize_parameters(n_x, n_h, n_y):
    ...
    parameters = {"W1": W1,
                  "W2": W2,}    
    return parameters
def forward_propagation_negSam(X, parameters,negSampleIndx,target):
    """
    Argument:
...
def skipgram_model_loop(X, Y, n_h):
    ...
    for word_indx in range(len(X)):
        ...
        if ConfigJsondata["hyperparameters"]["negSampl"] == 1:
                        ...
                        #print negSamples
        cache = forward_propagation_negSam(X[[word_indx],:], parameters,negSamples,hot_to_number(Y))
