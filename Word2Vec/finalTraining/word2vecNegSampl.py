...
class UnigramTable:
    ...
    def __init__(self, vocab,vocabDictCount):
        ...
        #print'----'
        #print norm
        table_size = 1000#100000000 # Length of the unigram table
...
for word_index, word in enumerate(corpus_raw[0:50]):
        ...
        while counter<WINDOW_SIZE:
            ...
            if word in vocabWordsDict:
                ...
                if corpus_raw[word_index-(counter)] in vocabWordsDict:
                    #print "------------------"
                    #print corpus_raw[word_index-(counter)]
                    vocabWordsDictCounter[corpus_raw[word_index-(counter)]]=vocabWordsDictCounter[corpus_raw[word_index-(counter)]]+1
...
def initialize_parameters(n_x, n_h, n_y):
    ...
    parameters = {"W1": W1,
                  "W2": W2,}    
    return parameters
def forward_propagation_negSam(X, parameters,negSampleIndx):
    """
    Argument:
...
def skipgram_model_loop(X, Y, n_h):
    ...
    if ConfigJsondata["hyperparameters"]["negSampl"] == 1:
        n_y = ConfigJsondata["hyperparameters"]["negaSampSize"] + 1
    else:
        n_y = layer_sizes(X, Y)[2] #==vocabsize=7
    ...
    for word_indx in range(len(X)):
        ...
        if ConfigJsondata["hyperparameters"]["negSampl"] == 1:
                        ...
                        #print negSamples
        cache = forward_propagation_negSam(X[[word_indx],:], parameters,negSamples)
        target = [1,0,0,0,0]#Y[[word_indx],:]#np.empty((1, vocabSize))
        #print cache["A2"]
        #print 
        errorArr=target-cache["A2"]
        #print "error shape is"
...
#def mainFunc():
print skipgram_model_loop(x_train,y_train,(ConfigJsondata["hyperparameters"]["dimen"]))["W1"]
#np.savetxt('vectors.txt', res)
